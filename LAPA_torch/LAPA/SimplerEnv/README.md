...
### LAPA PyTorch Adapter Evaluation

The `lapa-adapter` policy evaluates the PyTorch adapter checkpoints from
`../adapter_distill`; it does not load JAX `params::...` checkpoints. Fine-tune
the adapter on `../data/simpler.jsonl` first, then run:
...
`../adapter_distill/checkpoints/pythia160m_simpler/best.pt` and can be
overridden with `CKPT_PATH`.
